simulate_2slippers = function(alpha_pos, alpha_neg, beta, delta, gamma, rewardprob, day){
  ...
  #   DELTA - Perseverence parameter
  #   GAMMA - slippage parameter - dictates how much an animal forgets between sessions
  ...
  w1 = rep_len(0.5, ntrial)        #weight for option 1
  w2 = rep_len(0.5, ntrial)        #weight for option 2
  ...
  for (n in 1:ntrial){
    if (n > 1){ #compute log odds of choice for current trial
      if (day[n] != day[n-1]){
        w1[n] = gamma * w1[n] + (1-gamma)* 0.5
        w2[n] = gamma * w2[n] + (1-gamma)* 0.5
      }
      logodds[n] = beta * (w1[n] - w2[n] - delta * (1-2*choice[n-1]))
    }
    else {
      logodds[1] = 0
    }
