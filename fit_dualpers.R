fit_dualpers = function(inx, choice, reward){
  ...
  lb = c(0, 0, 0, -Inf)     #lower limits for alpha and beta
  ub = c(1.2, 1.2, 100, Inf) #upper limits
  inx0 = inx
  dof = length(inx)
  ...
  best = bestmodel(result$alpha_pos, result$alpha_neg, result$beta, result$delta, choice, reward) #best fitting model
...
bestmodel = function(alpha_pos, alpha_neg, beta, delta, choice, reward){
  #function to evaluate the loglikelihood of the model for parameters alpha
  #and beta given the data
  alpha_pos = alpha_pos
  alpha_neg = alpha_neg
