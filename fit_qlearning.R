fit_qlearning = function(inx, choice, reward){
...
lb = c(0, 0)     #lower limits for alpha and beta
ub = c(1.2, 100) #upper limits
inx0 = inx
dof = length(inx)
...
best = bestmodel(result$alpha, result$beta, choice, reward) #best fitting model
...
bestmodel = function(alpha, beta, choice, reward){
  #function to evaluate the loglikelihood of the model for parameters alpha
  #and beta given the data
  alpha = alpha
  beta = beta
