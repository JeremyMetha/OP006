fit_slippage = function(inx, choice, reward, day){
...
lb = c(0, 0, 0)     #lower limits for alpha, beta and gamma
ub = c(1.2, 100, 1) #upper limits
inx0 = inx
dof = length(inx)
...
best = bestmodel(result$alpha, result$beta, result$gamma, choice, reward, day) #best fitting model
...
bestmodel = function(alpha, beta, gamma, choice, reward, day){
  #function to evaluate the loglikelihood of the model for parameters alpha
  #and beta given the data
  alpha = alpha
  beta = beta
